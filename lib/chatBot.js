const ver = "1.0"; //update this before pushing to master.
...
var ChatBot = function(username, password, options) {
	...
	if (this.logFile===undefined||this.logFile===true) { this.logFile = "data/"+this.name+"/bot."+this.name+".log"; }
	
	this.keyFile = options.hasOwnProperty('keyFile') ? options.keyFile : this.name+"/bot."+this.name+"key.txt";
	if (this.keyFile===undefined||this.keyFile===true) { this.keyFile = "data/"+this.name+"/bot."+this.name+"key.txt"; }
	...
	this.unmutedState = steamClient.EPersonaState.Online;
	this.mutedState = steamClient.EPersonaState.Snooze;
	...
	this.steamClient.chat.on("friendMessage",      function(message)                          { that._onFriendMsg(message); });
...
ChatBot.prototype._onLogOnResponse = function(res) {
	var that = this;
	if(res.eresult === steamClient.EResult.OK) {
...
ChatBot.prototype._getEResult = function(num) {
	for (var result in steamClient.EResult) {
		if(steamClient.EResult[result]===num) {
			return result;
		}
	}
	return false;
...
ChatBot.prototype._onFriendMsg = function(message) {
	this.winston.info(this.name + "/chatBot.js: friendMsg <" + this._userString(message.steamid_friend) + ">: " + message.message);
	
	var haveSentMessage = false;
	_.each(this.triggers, function(trigger) {
		var sentMessageThisTrigger = trigger.onFriendMessage(message, haveSentMessage);
...
ChatBot.prototype._onChatStateChange = function(stateChange, chatterActedOn, steamChatId, chatterActedBy) {
	...
	if ((stateChange & steamClient.EChatMemberStateChange.Kicked) > 0) {
	...
	else if ((stateChange & steamClient.EChatMemberStateChange.Entered) > 0) {
	...
	else if ((stateChange & steamClient.EChatMemberStateChange.Left) > 0) {
	...
	else if ((stateChange & steamClient.EChatMemberStateChange.Disconnected) > 0) {
	...
	else if ((stateChange & steamClient.EChatMemberStateChange.Banned) > 0) {
