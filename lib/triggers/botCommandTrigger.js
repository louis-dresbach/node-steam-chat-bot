...
BotCommandTrigger.prototype._triggerOnMatch = function(fromId,toId,message) {
	if (this._messageTriggers(message.message) && this.options.callback) {
		if (this._checkUser(fromId)) {
			this._sendMessageAfterDelay(message, "Your wish is my command.");
			...
			this._sendMessageAfterDelay(message, "You are not allowed to use this command.");
			return false;
		}
	}
	return false;
