...
BotCommandTrigger.prototype._checkUser = function(fromId) {
...
BotCommandTrigger.prototype._triggerOnMatch = function(fromId,toId,message) {
	if (this._messageTriggers(message) && this.options.callback) {
		if (this._checkUser(fromId)) {
			this._sendMessageAfterDelay(fromId, "Your wish is my command.");
			...
			this._sendMessageAfterDelay(fromId, "You are not allowed to use this command.");
			return false;
		}
	}
	return false;
