...
RemoveFriendTrigger.prototype._stripCommand = function(message) {
	if (this.options.command && message && message.message.toLowerCase().indexOf(this.options.command.toLowerCase() + " ") === 0) {
		return {message: message.message, params: message.message.split(" ")};
	}
	return null;
}
