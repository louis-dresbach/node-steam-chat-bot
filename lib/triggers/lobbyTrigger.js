...
LobbyTrigger.prototype._respond = function(toId, message, fromId) {
	...
	if (this._checkMessage(message, matches)) {
		...
		.then((userCredential) => {
			this._auth(fromId, false).then((auth) => {
				var ref = firebase.database().ref('entries');
				ref.orderByChild("id").equalTo(fromId).once("value")
				.then((val) => {
	...
	if (this._checkMessage(message, rmatches)) {
		...
		.then((userCredential) => {
			this._auth(fromId, false).then((auth) => {
				var ref = firebase.database().ref('entries');
				var p = ref.orderByChild("id").equalTo(fromId).once("value")
				.then((val) => {
					if(typeof(val.val()) !== 'undefined' && val.val() != null) {
						val.forEach((childSnapshot) => {
	...
	if (this._checkMessage(message, aumatches)) {
		...
		firebase.auth().signInWithEmailAndPassword(fusername, fpassword)
		.then((userCredential) => {
			this._auth(fromId, true).then((auth) => {
				if (spl.length > 1) {
					var userId = spl[1];
					if (userId.length != 17) {
...
LobbyTrigger.prototype._auth = function(steamId, onlyAdmin) {
	this.chatBot.winston.debug("Trying to authorize " + this._username(steamId) + ".");
	return when.promise(function(resolve, reject) {	
		firebase.auth().signInWithEmailAndPassword(fusername, fpassword)
		.then((userCredential) => {
			var ref = firebase.database().ref('players/'+steamId).once("value")
			.then((val) => {
				if(typeof(val.val()) !== 'undefined' && val.val() != null) {
