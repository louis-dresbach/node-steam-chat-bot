...
BaseTrigger.prototype.onFriendMessage = function(message, haveSentMessage) {
	var that = this;
	var userId = message.steamid_friend;
...
// Return true if a message was sent
BaseTrigger.prototype.onChatMessage = function(message, haveSentMessage, muted) {
	var that=this;
	var chatterId = message.steamid_sender;
	var roomId = message.chat_id;
...
BaseTrigger.prototype._sendMessageAfterDelay = function(originalMessage, message) {
	var that = this;
	if (this.options.delay) {
		// If message was sent in group chat
		if(originalMessage.hasOwnProperty('chat_group_id')) {
			this.winston.silly(this.chatBot.name+"/"+this.name,{"trigger":this.name,"target: ":originalMessage.chat_group_id,"message":message, "delay":this.options.delay});
			setTimeout(function () {
				that.winston.silly(that.chatBot.name+"/"+that.name+": sending delayed message to "+steamId+": "+message);
				that.chatBot.steamClient.chat.sendChatMessage(originalMessage.chat_group_id, originalMessage.chat_id, message);
			}, this.options.delay);
			
		}
		// It was a private message
		else {
			var steamId = originalMessage.steamid_friend;
			this.winston.silly(this.chatBot.name+"/"+this.name,{"trigger":this.name,"target: ":steamId,"message":message, "delay":this.options.delay});
			setTimeout(function () {
				that.winston.silly(that.chatBot.name+"/"+that.name+": sending delayed message to "+steamId+": "+message);
		...
		if(originalMessage.hasOwnProperty('chat_group_id')) {			
			this.winston.silly(that.chatBot.name+"/"+that.name+": sending nondelayed message to "+originalMessage.chat_group_id+": "+message);
			this.chatBot.steamClient.chat.sendChatMessage(originalMessage.chat_group_id, originalMessage.chat_id, message);
		}
		// It was a private message
		else {
			var steamId = originalMessage.steamid_friend;
			this.winston.silly(that.chatBot.name+"/"+that.name+": sending nondelayed message to "+steamId+": "+message);
...
BaseTrigger.prototype._checkMessage = function(message, matches) {
...
// Authorize using firebase
BaseTrigger.prototype._firebaseAuth = function(steamId, onlyAdmin) {
	this.chatBot.winston.debug("Trying to authorize " + this._username(steamId) + " using firebase.");
	return when.promise(function(resolve, reject) {	
		firebase.auth().signInWithEmailAndPassword(fusername, fpassword)
		.then((userCredential) => {
			var ref = firebase.database().ref('players/'+steamId.getSteamID64()).once("value")
			.then((val) => {
				if(typeof(val.val()) !== 'undefined' && val.val() != null) {
					...
					if(val.val().role == "user" && !onlyAdmin) {
