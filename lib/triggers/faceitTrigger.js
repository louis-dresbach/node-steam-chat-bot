...
FaceitTrigger.prototype._getAndSend = function (steamId, faceitId) {
	...
	.then((d) => {
		...
		this._sendMessageAfterDelay(steamId, message);
	}).catch((error) => {
		console.error(error);
		...
		.then((userCredential) => {
			...
			.then((val) => {
				if(typeof(val.val()) !== 'undefined' && val.val() != null && typeof(val.val().faceit) !== 'undefined') {
					...
					this._getAndSend(fromId, val.val().faceit);
				}
				else {
					...
					.then((data) => {
						if(data.items.length == 1) {
							...
							this._getAndSend(fromId, data.items[0].player_id);
						}
						else {
